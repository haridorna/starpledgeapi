...
class YelpSearch {
    ...
    function __construct( $adapter, $serviceLocator, $path="search"){
        ...
        /*
        $this->consumer_key     = $this->yelpConfig['api']['yelp']['consumer_key'];     // "v-0-jjdbK5ssZay_LIlMNg";
        $this->consumer_secrete = $this->yelpConfig['api']['yelp']['consumer_secret'];  // "VnLES-k5m3h-ClQeKgGXZ0EAHgQ";
        $this->token            = $this->yelpConfig['api']['yelp']['token'];            // "D5vCcXtP-aJDoNj36b6XMvFiZbEsybU1";
        $this->token_secret     = $this->yelpConfig['api']['yelp']['token_secret'];     // "y2xItiPD8kTQ2OpAR3JGQw6_CzY";
        */
        $max_number = 2;
        $randmo_number = rand(0, $max_number);

        list($this->consumer_key,$this->consumer_secrete, $this->token ,$this->token_secret ) = array_values($this->yelpConfig['group-api']['yelp'][$randmo_number]);
