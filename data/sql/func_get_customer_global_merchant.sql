...
drop function if exists `func_get_customer_global_merchant`;
delimiter //
...
BEGIN
  ...
  DECLARE _cursor_vp CURSOR 
    ...
    where 
      cq.customer_id=CustomerIDSelect and cq.global_merchant_id=GlobalMerchantIDSelect and som.option_text is not NULL
  ...
  set _resultprocedure= CONCAT(_resultprocedure,_select_concat); 	     
      ...
      set _resultprocedure= CONCAT(_resultprocedure,_select_concat,',');     
       ...
       select concat('"like_status":"',if(count(*), 'Yes','No'),'"')
          into _select_concat  
        from merchant_user_likes mul, merchant m where mul.customer_id=CustomerIDSelect and m.global_merchant_id=GlobalMerchantIDSelect and mul.merchant_id=m.id;             
       set _resultprocedure= CONCAT(_resultprocedure,_select_concat,',');     
