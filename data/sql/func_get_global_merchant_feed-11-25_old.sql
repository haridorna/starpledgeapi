CREATE DEFINER=`privpass_admin`@`%` FUNCTION `func_get_global_merchant_feed`(`GlobalMerchantIDSelect` BIGINT, `IsBefore` SMALLINT, `TimeStampSelect` TIMESTAMP, `Feed_limit` INT, `Nest_limit` INT)
  ...
  DECLARE CONTINUE HANDLER FOR NOT FOUND SET _done = TRUE;
   ...
   if _addmaincomma then
      ...
      select  concat('"spending_power":"',COALESCE(max(rank)*10,0), '%","spanding_power_level":"',
        ...
        left join stat_global_merchant_category_unrolled on stat_global_merchant_category_unrolled.category_id=rank_customer_category_sum.customer_id 
      ...
      set _resultprocedure= CONCAT(_resultprocedure,_select_concat,',');     
       ...
       select CONCAT('"loyalty_rank":"',percentile,'","is_top_customer":"',100-percentile,'%"')
          into _select_concat 
       ...
       select concat('"average_check":"',COALESCE(avg_transaction,0),'","transactions_at_restaurant":"',COALESCE(number_of_transactions,0),'",')
        into _select_concat 
       ...
       select concat('"statistics": "',coalesce(name,''),'","avg_transaction_amount": "$,',coalesce(max(monthly_avg),0),'",')
        into _select_concat  
       from stat_customer_category scc
        left join stat_global_merchant_category_unrolled sgmcu on sgmcu.category_id=scc.customer_id 
        left join business_category bc on bc.id = scc.category_id
       where  scc.customer_id=_customer_id 
      ...
      set _select_concat=''; 
       ...
       select (exists(select CONCAT('"total_loyalty_points":"',total_cashback_earned*10,
                '","total_purchases":"$',total_purchases*10, 
